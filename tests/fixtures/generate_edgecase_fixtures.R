...
set.seed(123)  # same seed as medium fixture
np9 <- 100; ns9 <- 12
data_med <- matrix(rnorm(np9 * ns9, mean = 10, sd = 2), nrow = np9)
data_med[, 5:8]  <- data_med[, 5:8]  + 1.8
data_med[, 9:12] <- data_med[, 9:12] - 1.2
missing_mask <- matrix(FALSE, nrow = np9, ncol = ns9)
for (i in seq_len(np9)) {
  if (runif(1) < 0.3) {
...
rownames(data_med) <- paste0("protein_", seq_len(np9))
colnames(data_med) <- paste0("sample_", seq_len(ns9))
batch_med <- rep(1:3, each = 4)
desc_med <- data.frame(ID = colnames(data_med), sample = seq_len(ns9), batch = batch_med)
...
tryCatch({
  result <- harmonizR(
    ...
    output_file = file.path(out_dir, "medium_harmonizr_limma"),
    verbosity = 0, cores = 1
  )
